...
const createCandle = async (
  ...
  try {
    ...
    const blackImage1 = await black4oz.build({ format: "png" });
    const blackFileName1 = generateUniqueFileName();
    const blackFilePath1 = path.join(
      __dirname,
      "../../public/assets/images",
      blackFileName1
    );

    fs.writeFileSync(blackFilePath1, blackImage1);

    const blackBase64Image1 = await getBase64FromFile(blackFilePath1);

    const blackImageId1 = await uploadImageToPrintify(
      blackFileName1,
      blackBase64Image1,
      token
    );

    const blackImage2 = await black9oz.build({ format: "png" });
    const blackFileName2 = generateUniqueFileName();
    const blackFilePath2 = path.join(
      __dirname,
      "../../public/assets/images",
      blackFileName2
    );

    fs.writeFileSync(blackFilePath2, blackImage2);

    const blackBase64Image2 = await getBase64FromFile(blackFilePath2);

    const blackImageId2 = await uploadImageToPrintify(
      blackFileName2,
      blackBase64Image2,
      token
    );
    ...
    if (data) {
      fs.unlinkSync(blackFilePath1);
      fs.unlinkSync(blackFilePath2);
